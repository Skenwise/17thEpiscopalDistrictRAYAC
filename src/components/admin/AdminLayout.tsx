...
import {
  Users, Calendar, BookOpen, Handshake, Download,
  Heart, Shield, LogOut, Menu, X,
} from 'lucide-react';
import { useState } from 'react';

type AdminSection = 'members' | 'rsvps' | 'enrollments' | 'volunteers' | 'resources' | 'prayers' | 'admins';
...
const menuItems = [
  ...
  { id: 'resources' as AdminSection, label: 'Resources Manager', icon: <Download className="w-5 h-5" /> },
...
export default function AdminLayout({ children, activeSection, onSectionChange }: AdminLayoutProps) {
  ...
  const SidebarContent = () => (
    <div className="p-6 space-y-6 h-full flex flex-col">
      <div>
        <h1 className="text-2xl font-bold font-heading text-white mb-1">Admin Panel</h1>
      ...
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-400 hover:text-accent-red hover:bg-accent-red/10 transition-all"
      >
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col w-64 flex-shrink-0 bg-slate-800 border-r border-slate-700">
        <SidebarContent />
      </aside>
      ...
      {/* Mobile Sidebar */}
      {sidebarOpen && (
        <aside className="fixed w-64 h-screen bg-slate-800 border-r border-slate-700 z-40 lg:hidden">
          <SidebarContent />
        </aside>
      )}
